...
const _data: AudioData = {
  bass: 0, mid: 0, treble: 0, loudness: 0, beat: 0,
  isActive: false,
  freqBins: new Float32Array(128),
};

let _sBass = 0, _sMid = 0, _sTreble = 0, _sBeat = 0; // smoothed values
let _rafId = 0;
let _bpm = 120;
let _lastTrackId = '';
...
function sn(t: number, f: number): number {
  return (
    Math.sin(t * f * 6.2832) * 0.50 +
    Math.sin(t * f * 6.2832 * 1.73) * 0.30 +
    Math.sin(t * f * 6.2832 * 2.39) * 0.20
  ) * 0.5 + 0.5;
}
...
function tick() {
  ...
  if (!active) {
    ...
    const tBass = 0.18
      + (isKick  ? kickDecay * 0.66 : Math.exp(-9 * beatFrac) * 0.14)
      + sn(t, 0.16) * 0.09;

    // ── Beat impulse: very sharp at kick onset ──────────────────
    const tBeat = isKick && beatFrac < 0.035 ? 1.0
      : kickDecay * (isKick ? 0.45 : 0.08);

    // ── Mid: snare transient + harmonic sustain ─────────────────
    const snareDecay = Math.exp(-6 * beatFrac);
    const tMid = 0.22
      + (isSnare ? snareDecay * 0.44 : 0)
      + sn(t, 0.11) * 0.15;
